/* Il motore dei preventivi.
 *
 * Questo codice stava dentro app/api/spedizioni/tariffe/route.ts. E' stato SPOSTATO qui
 * senza riscriverne una riga: l'unica differenza sono i `return`, che prima costruivano
 * una risposta HTTP e ora restituiscono un valore.
 *
 * Perche': serviva un preventivo anche al SITO PUBBLICO, dove non c'e' nessuna sessione.
 * Riscrivere il calcolo li' avrebbe creato due verita' sui prezzi, e il giorno che se ne
 * cambia una l'altra resta indietro in silenzio. Qui la regola e' una sola e la chiamano
 * in due: la rotta del portale (con la sessione) e quella pubblica (che forza il Listino
 * Standard Web di MULTIEXPRESS).
 *
...
export const superaMisureMax = (settings: any, pesoReale: number, colli: any[]): boolean =>
...
export function descriviLimiti(settings: any, pesoReale?: number): string {
  if (!settings) return ''
  const parts: string[] = []
...
export type EsitoTariffe =
  | { risultati: any[]; errore?: undefined; stato?: undefined }
  | { risultati?: undefined; errore: string; stato: number }

export async function calcolaTariffeCliente(
  supabase: any,
  ctx: { cliente: any; clienteId: string | null; subMatch: string | null },
  body: any,
): Promise<EsitoTariffe> {
  const { cliente, clienteId, subMatch } = ctx
  ...
  if (!cliente.listino_cliente_id) {
    return { errore: 'Nessun contratto attivo', stato: 400 }
  }
  ...
  if (errFasce) {
    ...
    return { errore: 'Errore tecnico nel calcolo delle tariffe: riprova tra poco. Se persiste contatta l\'assistenza (il listino non c\'entra).', stato: 500 }
  }
  if (!fasce?.length) {
    return { errore: 'Listino vuoto — configura le fasce prezzi', stato: 400 }
  }
