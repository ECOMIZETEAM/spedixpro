...
export default function ReportSpedizioniPage() {
  ...
  const STATI: Record<string, string> = {
    creata: 'Creata', in_transito: 'In transito', consegnata: 'Consegnata',
    giacenza: 'Giacenza', annullata: 'Annullata', reso: 'Reso',
  }
    ...
    <div>
      ...
      <div style={{background:'#fff',borderRadius:'8px',border:'1px solid #d1d5db',overflow:'hidden'}}>
        ...
        <table style={{width:'100%',borderCollapse:'collapse' as const,fontSize:'13px'}}>
          ...
          <tbody>
            {!reports.length ? (
              <tr><td colSpan={8} style={{padding:'40px',textAlign:'center' as const,color:'#999'}}>Nessun report generato</td></tr>
              ...
              <tr key={r.id} style={{borderBottom:'1px solid #f0f0f0'}}>
                ...
                <td style={{padding:'14px 16px',color:'#f97316',fontSize:'12px'}}>
                  {Object.entries(r.filtri||{}).filter(([,v])=>v).map(([k,v])=>`${k}=${v}`).join(', ') || '-'}
                </td>
                <td style={{padding:'14px 16px'}}>
                  <span style={{background:'#e0f2fe',color:'#0369a1',padding:'3px 10px',borderRadius:'4px',fontSize:'11px',fontWeight:'700'}}>{(r.formato||'').toLowerCase()}</span>
                </td>
