...
export default function ReportResiPage() {
  ...
  async function salvaReport(fileBase64: string, nomeFile: string, formato: string) {
    ...
    const j = await inviaReport({ tipo: 'resi', filtri: filtriTxt, formato, fileBase64, nomeFile, clienteId: filtri.clienteId || null })
  ...
  async function generaReport() {
    ...
    if (formato === 'xlsx' || formato === 'csv') {
      ...
      const rows = spedizioni.map((s:any) => ({
        ...
        'Destinatario': s.dest_nome, 'Città': s.dest_citta,
      ...
      const doc = new jsPDF()
      doc.setFontSize(14)
      doc.text(`Report Resi — ${filtri.dal} / ${filtri.al}`, 14, 15)
      autoTable(doc, {
        startY:25,
        head:[['N. Spedizione','Cliente','Destinatario','Città','Data','Totale €']],
        body: spedizioni.map((s:any)=>[
          s.numero, s.clienti?.ragione_sociale||s.mitt_nome, s.dest_nome,
          s.dest_citta, new Date(s.created_at).toLocaleDateString('it-IT'),
  ...
  return (
    <div>
      <div style={{marginBottom:'16px'}}><h1 style={{fontSize:'20px',fontWeight:'700',color:'#1a1a1a',margin:0}}>Genera Report Resi</h1></div>
