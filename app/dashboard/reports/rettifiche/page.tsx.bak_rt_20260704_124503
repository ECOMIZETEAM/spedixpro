...
export default function ReportRettifichePage() {
  ...
  async function generaReport() {
    ...
    const doc = new jsPDF()
    doc.setFontSize(14)
    doc.text(`Report Rettifiche — ${filtri.dal} / ${filtri.al}`, 14, 15)
    autoTable(doc, { startY:25, head:[['Cliente','Data','Vettore','Formato']], body:[[filtri.clienteId||'Tutti',`${filtri.dal} - ${filtri.al}`,filtri.vettore||'Tutti',filtri.formato]], styles:{fontSize:9}, headStyles:{fillColor:[249,115,22]} })
    doc.save(`report_rettifiche_${filtri.dal}.pdf`)
    await fetch('/api/reports/spedizioni',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({formato:filtri.formato,filtri,tipo:'rettifiche'})})
    fetch('/api/reports/lista?tipo=rettifiche').then(r=>r.json()).then(d=>setReports(d||[]))
    setGenerating(false)
  }
