...
// Etichettina della SECONDA riga (contratto/motivo/importi sotto): piccola e tenue, non deve pesare.
...
export default function GiacenzePage() {
  ...
  const [filtri, setFiltri] = useFiltriPersistenti('giacenze-master:filtri', filtriDefault())
              ...
              <tbody>
                {giacenzePaginate.map(g => {
                  ...
                  const secItems: { l: any; v: any }[] = [
                    ...
                    { l: 'Totale', v: <span>{costo > 0 ? <span style={{color:'#dc2626',fontWeight:'600'}}>€ {costo.toFixed(2)}</span> : '0.00 €'}{giorni > 1 && <span style={{fontSize:'10px',color:'#1a1a1a',marginLeft:'4px'}}>({giorni} giorni)</span>}</span> },
