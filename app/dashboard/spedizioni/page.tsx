...
const FILTRI_DEFAULT = {
  ...
  // Ordinamento colonna (vuoto = data più recente). L'ordine è applicato a DB (server-side) su tutto
  // il periodo filtrato, non solo sulla pagina: la spedizione più pesante esce in cima anche se sta
...
export default function SpedizioniPage() {
  ...
  const [correggiOpen, setCorreggiOpen] = useState(false)   // apre il dettaglio già in modalità "Correggi spedizione"
  ...
  function buildParams(paginaReq: number) {
    ...
    // Selezione cliente/sotto-master: parametri dedicati (rete/clienteEq) per mantenere lo scope
    // di rete e quindi gli stessi prezzi/margini di prima.
    if (filtri.clienteId) {
      if (filtri.clienteId.startsWith('m:')) q.set('rete', filtri.clienteId.slice(2))
      else q.set('clienteEq', filtri.clienteId)
    }
    for (const k of ['stato','contratto','vettore','negozio','agente','id_ordine','contrassegno','stato_contrassegni','assicurazione','fatturato','dest_citta','dest_cap','contenuto'] as const) {
                    ...
                    <tr style={{background:isSelected?'#fff7ed':'#fff'}}>
                      ...
                      <td style={{padding:'9px 12px 3px'}}>
                        <div style={{display:'flex',gap:'4px'}}>
                          ...
                          {s.cliente_id && s.stato!=='annullata' && <button onClick={()=>{ setCorreggiOpen(true); setDettaglio(s) }} title="Correggi spedizione: peso, misure, contenuto e note (cambiando peso o misure si ricalcola il costo)" style={{padding:'4px 8px',background:'#fff7ed',color:'#ea580c',borderRadius:'4px',fontSize:'14px',border:'1px solid #fed7aa',cursor:'pointer'}}>✏️</button>}
