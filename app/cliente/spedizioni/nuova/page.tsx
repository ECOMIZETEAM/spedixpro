...
interface Tariffa { carrierCode:string; contractCode:string; total_price:string; zona:string; peso_fatturato:string; peso_reale:number; peso_volume:string; prezzo_spedizione?:string; weight_price?:string; costo_sponda?:string; costo_fuel?:string; fuel_pct?:number; costo_contrassegno?:string; costo_assicurazione?:string; accessori_disponibili?:{nome:string;prezzo:number;perc:number}[]; limiti_collo?:string; avviso_fuori_sagoma?:string; _corriere_id?:string; _corriere_tipo?:string; _brt_assegno?:boolean; corriere_nome?:string; _deposito?:boolean; _consegna_punto?:string }
...
export default function NuovaSpedizioneCliente() {
  ...
  async function calcolaTariffe() {
    if (consegnaA==='punto' ? (!dest.nome||!dest.cap||!dest.telefono) : (!dest.nome||!dest.indirizzo||!dest.citta||!dest.cap||!dest.telefono)) {
      setErrore(consegnaA==='punto' ? 'Per la consegna a un punto servono nome, CAP e telefono del destinatario.' : 'Compila tutti i dati del destinatario (incluso il telefono)'); return
    }
    if (consegnaA==='domicilio' && dest.paese==='IT' && !dest.provincia) {
            ...
            <div style={cardB}>
              ...
              {isExtraUe(dest.paese) && (
                <div style={{marginTop:'10px',background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:'6px',padding:'10px 12px',fontSize:'12px',color:'#9a3412',lineHeight:1.5}}>
                  ...
                  es. EORI, da caricare a parte). Le destinazioni UE partono invece regolarmente.
                </div>
              )}
            </div>
          </div>)}
                ...
                return (
                  <div key={i} onClick={()=>setSelected(r)}
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      {r.avviso_fuori_sagoma && (
                        <div style={{fontSize:'10.5px',color:'#b45309',marginTop:'2px'}}>⚠ {r.avviso_fuori_sagoma}</div>
                      )}
                    </div>
