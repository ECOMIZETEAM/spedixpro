...
export default function SpedizioniPage() {
  ...
  // ETICHETTA DI RESO: crea le spedizioni di ritorno (mitt/dest invertiti, no contrassegno) e scarica le nuove etichette.
  async function confermaReso() {
    ...
    try {
      ...
      // Niente PDF unico: i resi compaiono in elenco e si scarica l'etichetta di ognuno singolarmente.
      ...
      if (ritFalliti.length) msg += ` Ritiro non prenotato per ${ritFalliti.length}.`
      setNotifica(msg); setTimeout(() => setNotifica(''), 6000)
      ricarica()
...
async function apriTracking(s: any) {
  ...
  async function elimina(id: string, numero: string) {
    ...
    if (res.ok && j.success) {
      ...
      // Mostra il motivo VERO dell'API (es. "Non annullabile: oltre 15 giorni"). Il messaggio generico
      // sui permessi resta solo come ultima spiaggia (403 senza corpo).
                  ...
                  return (
                    <tr key={s.id} style={{borderBottom:'1px solid #d1d5db',background:isSelected?'#fff7ed':'#fff'}}>
                      ...
                      <td style={{padding:'9px 12px'}}>
                        ...
                        {s.dogana_bloccata_at && !s.tracking_number && <div><span title="Bloccata in dogana: servono EORI, contenuto dettagliato ed email del destinatario. Controlla il ticket che ti abbiamo aperto."
