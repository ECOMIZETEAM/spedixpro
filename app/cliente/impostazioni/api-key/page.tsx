        ...
        {contratti.map((c:any)=>{
          ...
          return (
            <div key={c.id} style={card}>
              ...
              {ks.map((k:any)=>(
                <div key={k.id} style={{display:'flex',alignItems:'center',gap:'8px',padding:'10px 0 2px',marginTop:'8px',borderTop:'1px solid #f0f0f0'}}>
                  <code style={{flex:1,fontSize:'12.5px',color:'#1a1a1a',wordBreak:'break-all',background:'#f9fafb',border:'1px solid #eee',borderRadius:'4px',padding:'7px 9px'}}>{k.chiave}</code>
                  <button onClick={()=>copia(k.chiave)} style={{background:ACCENT,color:'#fff',border:'none',borderRadius:'6px',padding:'7px 12px',fontSize:'12px',fontWeight:700,cursor:'pointer'}}>Copia</button>
                  <button onClick={()=>revoca(k.id)} style={{background:'#fef2f2',color:'#dc2626',border:'1px solid #fecaca',borderRadius:'6px',padding:'7px 10px',fontSize:'12px',fontWeight:600,cursor:'pointer'}}>Elimina</button>
                </div>
              ))}
            </div>
          )
