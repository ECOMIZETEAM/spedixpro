...
export default function DistinteResiPage() {
  ...
  async function carica() {
    ...
    const res = await fetch(`/api/cliente/resi/distinte?${params}`)
                ...
                <tr key={d.id} style={{borderBottom:'1px solid #d1d5db'}}>
                  ...
                  <td style={{padding:'10px 14px'}}>
                    <div style={{display:'flex',gap:'6px'}}>
                      <button onClick={()=>window.open(`/cliente/resi/distinte/${d.id}`,'_blank')}
                      ...
                      <button onClick={()=>window.open(`/api/cliente/resi/distinte/${d.id}/pdf`,'_blank')}
