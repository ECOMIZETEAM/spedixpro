...
export async function POST(req: NextRequest) {
  ...
  if (!isProprio && cliente?.listino_cliente_id) {
    ...
    if (!masterSub) {
      ...
      const esente = await clienteEsentePavimento(adminCrea, clienteId)
      const bandePav = esente ? [] : await pavimentoContratto(adminCrea, (corriereRecord as any).nome_contratto, (risPrezzo as any).zona)
  ...
  if (corriereRecord.tipo === 'spedisci') {
    ...
    const res = await fetch(`${baseUrl}/shipping/create`, {
      ...
      body: JSON.stringify({
        ...
        //    Spedisci/Poste, non nel raw_response): lo scriviamo col RE-PRINT del PDF a serve-time
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // GLS DIRETTO — web service proprio del master (contratto GLS diretto)
  //
  // Sincrono: AddParcel torna subito il NumeroSpedizione, GetPdfBySped l'etichetta PDF (struttura
  // e flusso VERIFICATI contro l'API vera — vedi lib/gls.ts). Il prezzo NON viene da GLS: come il
  // circuito interno arriva dal LISTINO (il master paga GLS per conto proprio; MoovExpress addebita
  // solo la fee sui contratti propri, via trigger). Percio' il costo si calcola PRIMA di creare, cosi'
  // dopo la creazione l'unico punto che puo' fallire e' l'inserimento a DB — e li' si ANNULLA su GLS
  // (DeleteSped) per non lasciare la spedizione orfana sul corriere.
  // ═══════════════════════════════════════════════════════════════════════════
  if (corriereRecord.tipo === 'gls') {
    const credGls = {
      sigla_sede: cred.sigla_sede, user_webservice: cred.user_webservice,
      password_webservice: cred.password_webservice, codice_contratto: cred.codice_contratto,
    }
    if (!credGls.sigla_sede || !credGls.user_webservice || !credGls.password_webservice) {
      await stornaPrenotazione()
    ...
    try {
      ...
      if (!ris.numeroSpedizione) {
        // AddParcel non ha creato nulla: niente da annullare, si libera la prenotazione.
        await stornaPrenotazione()
      ...
      after(async () => {
        try {
          ...
          await inviaEmailSpedizioneCreata({
            ...
            mittNome: body.shipFrom?.name, destNome: body.shipTo?.name, destCitta: body.shipTo?.city,
            numero: numeroFinale, corriere: corriereRecord.nome_contratto, notificaDest,
            spedizioneId: inserted?.id || null, masterId,
          })
        } catch { /* la spedizione e' gia' creata: l'email non blocca nulla */ }
      })
      ...
      console.error('[CREA][GLS]', err?.message)
      await stornaPrenotazione()
  ...
  if (corriereRecord.tipo === 'fedex') {
    const credFedex = {
      api_key: cred.api_key, api_secret: cred.api_secret, account_number: cred.account_number,
      track_api_key: cred.track_api_key, track_api_secret: cred.track_api_secret,
    }
    if (!credFedex.api_key || !credFedex.api_secret || !credFedex.account_number) {
      await stornaPrenotazione()
      return NextResponse.json({ error: 'Contratto FedEx non configurato correttamente. Contatta l\'assistenza.' }, { status: 400 })
    }
    // serviceType/pickupType/test stanno nel contratto (settings). Senza serviceType FedEx rifiuta.
    const settingsFx = (corriereRecord as any)?.settings || {}
    const serviceType = String(settingsFx.tipo_servizio || '').trim()
    if (!serviceType) {
      await stornaPrenotazione()
      return NextResponse.json({ error: 'Contratto FedEx senza tipo di servizio impostato. Contatta l\'assistenza.' }, { status: 400 })
    }
    try {
      const { creaSpedizioneFedex, annullaSpedizioneFedex } = await import('@/lib/fedex')
      ...
      // Un collo per elemento (peso + misure). FedEx crea un requestedPackageLineItem per collo.
      const colliFx = packages.map((p: any) => ({
        pesoKg: parseFloat(p?.weight) || 1,
        lunghezza: parseFloat(p?.length) || undefined,
        larghezza: parseFloat(p?.width) || undefined,
        altezza: parseFloat(p?.height) || undefined,
      }))
      // SATURDAY_DELIVERY se il cliente sceglie il servizio "Sabato". COD: contanti se incassoModalita 'C'
      // (default), fondi garantiti (assegno) altrimenti.
      const sabatoFx = (serviziAccessori || []).some((x: any) => /sabato|saturday/i.test(String(x?.nome || '')))
      const codContantiFx = String(body.incassoModalita || 'C') === 'C'

      const ris = await creaSpedizioneFedex(credFedex, {
        mittRagioneSociale: body.shipFrom.name, mittIndirizzo: body.shipFrom.street1,
        mittLocalita: body.shipFrom.city, mittProvincia: body.shipFrom.state, mittCap: body.shipFrom.postalCode,
        mittPaese: 'IT', mittContatto: body.shipFrom.name, mittTelefono: body.shipFrom.phone || undefined,
        ragioneSociale: body.shipTo.name, indirizzo: body.shipTo.street1,
        localita: body.shipTo.city, cap: body.shipTo.postalCode, provincia: body.shipTo.state,
        paese: body.shipTo.country || 'IT',
        contatto: body.shipTo.name, telefono: body.shipTo.phone || undefined, email: body.shipTo.email || undefined,
        colli: colliFx,
        importoContrassegno: body.codValue ? Number(body.codValue) : undefined,
        codContanti: codContantiFx,
        valoreDoganale: body.insuranceValue ? Number(body.insuranceValue) : undefined,
        contenuto: body.contenuto ? String(body.contenuto) : undefined,
        note: body.notes ? String(body.notes) : undefined,
        rifOrdine: (body.rifOrdine ? String(body.rifOrdine) : '').trim().substring(0, 30) || undefined,
        serviceType,
        pickupType: settingsFx.tipo_ritiro ? String(settingsFx.tipo_ritiro) : undefined,
        sabato: sabatoFx,
        test: settingsFx.test_mode === true,
      })
      ...
      // L'etichetta FedEx torna GIÀ nella create (una per collo): niente chiamata separata come il GLS.
      let etichettaUrl: string | null = null
      const etichettePerCollo = ris.etichette.map(b => `data:application/pdf;base64,${b}`)
      if (ris.etichette.length) {
        const { unisciEtichette } = await import('@/lib/easyparcel')
      ...
      const { data: inserted, error: insertError } = await supabase.from('spedizioni').insert({
        ...
        etichetta_url: etichettaUrl,
        colli_dettaglio: colliDettaglio,
        // _fedex marca il contratto proprio; trackingNumber serve all'annullo e al tracking.
        raw_response: {
          _fedex: true, trackingNumber: numeroFinale, trackingNumbers: ris.trackingNumbers,
          test: settingsFx.test_mode === true, warning: ris.warning || undefined,
        },
        stato: 'in_lavorazione',
        costo_spedizione: costoCorrente, costo_totale: costoCliente,
        servizi_accessori: serviziAccessori,
      ...
      if (insertError) {
        // La spedizione esiste su FedEx ma non da noi. FedEx auto-conferma: si annulla per non lasciare
        // un pacco fantasma che FedEx ritirerebbe. Best-effort in background.
        console.error('[CREA][FEDEX][INSERT]', numeroFinale, insertError.message)
        const testFx = settingsFx.test_mode === true
        after(async () => {
          try {
            const a = await annullaSpedizioneFedex(credFedex, numeroFinale, testFx)
            if (a.ok) console.log('[CREA][FEDEX] pacco fantasma annullato', numeroFinale)
            else console.error('[CREA][FEDEX] pacco fantasma NON annullato, annullare a mano', numeroFinale, a.errore)
          } catch (e) { console.error('[CREA][FEDEX] annullo fantasma:', e) }
        })
        return NextResponse.json({
          error: `Spedizione non registrata a sistema (rif. ${numeroFinale}): riprova. Se ricompare, contatta l'assistenza.`,
          numero: numeroFinale,
      ...
      console.error('[CREA][FEDEX]', err?.message)
      await stornaPrenotazione()
      return NextResponse.json({ error: erroreCorrierePulito(err?.message) }, { status: 400 })
    }
  }
