...
import { sincronizzaOrdiniTiktok } from '@/lib/tiktokSync'
...
export async function POST(req: NextRequest) {
  ...
  const { data: integr } = await supabase
    ...
    .eq('id', integrazioneId).eq('cliente_id', utente.cliente_id).eq('piattaforma', 'tiktok')
    .maybeSingle()
  if (!integr) return NextResponse.json({ error: 'Integrazione non trovata' }, { status: 404 })

  try {
    const res = await sincronizzaOrdiniTiktok(supabase, integr)
