...
import { leggiPrezzi, calcolaCosti } from '@/lib/giacenza-prezzi'
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: sped } = await admin.from('spedizioni')
    .select('id,master_id,cliente_id,corriere_id,stato,giacenza_stato,costo_totale,assicurazione').eq('id', id).maybeSingle()
  ...
  const costi = calcolaCosti(action, prezzi, sped)
