          ...
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '10px', width: '460px', maxWidth: '100%' }}>
            ...
            {modo === '' ? (
              ...
              <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <div><label style={{ fontSize: '12px', fontWeight: 600, color: '#1a1a1a', display: 'block', marginBottom: '5px' }}>{modo === 'pod' ? 'Note (facoltativo)' : 'Descrizione del problema'}</label>
                  <textarea value={desc} onChange={e => setDesc(e.target.value)} rows={modo === 'pod' ? 2 : 4} autoFocus placeholder={modo === 'pod' ? 'Eventuali note…' : 'Descrivi il problema su questa spedizione…'} style={{ width: '100%', padding: '9px 11px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '13px', color: '#1a1a1a', boxSizing: 'border-box', resize: 'vertical' }} /></div>
