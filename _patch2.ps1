$fp = "C:\Users\Acer-Swift 3\Desktop\spedixpro_completo\spedixpro\app\dashboard\reports\giacenze\page.tsx"
...
if ($cnt -eq 1) {
  $pager = "</table>" + $nl + "        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',borderTop:'1px solid #e5e7eb',gap:'8px'}}>" + $nl + "          <span style={{fontSize:'12px',color:'#666'}}>{reports.length} risultati</span>" + $nl + "          <div style={{display:'flex',gap:'4px'}}>" + $nl + "            <button onClick={()=>setPagina(p=>Math.max(1,p-1))} disabled={paginaCorr<=1} style={{padding:'5px 10px',border:'1px solid #d1d5db',borderRadius:'5px',background:'#fff',fontSize:'12px',cursor:'pointer',color:paginaCorr<=1?'#ccc':'#1a1a1a'}}>Precedente</button>" + $nl + "            {Array.from({length: totalePagine}, (_,i)=>i+1).filter(n => n===1 || n===totalePagine || Math.abs(n-paginaCorr)<=2).map((n)=>(" + $nl + "              <button key={n} onClick={()=>setPagina(n)} style={{minWidth:'30px',padding:'5px 8px',border:'1px solid',borderColor:n===paginaCorr?'#f97316':'#d1d5db',borderRadius:'5px',background:n===paginaCorr?'#f97316':'#fff',color:n===paginaCorr?'#fff':'#1a1a1a',fontSize:'12px',cursor:'pointer'}}>{n}</button>" + $nl + "            ))}" + $nl + "            <button onClick={()=>setPagina(p=>Math.min(totalePagine,p+1))} disabled={paginaCorr>=totalePagine} style={{padding:'5px 10px',border:'1px solid #d1d5db',borderRadius:'5px',background:'#fff',fontSize:'12px',cursor:'pointer',color:paginaCorr>=totalePagine?'#ccc':'#1a1a1a'}}>Successivo</button>" + $nl + "          </div>" + $nl + "        </div>"
  $c = $c.Replace("</table>", $pager)
} else { $err += ("table" + $cnt) }
if ($err.Count -gt 0) { Write-Host ("ANNULLATO - " + ($err -join ",")); exit }
Copy-Item $fp ($fp + ".bak_p2")
[System.IO.File]::WriteAllText($fp, $c, [System.Text.UTF8Encoding]::new($false))
$v = [System.IO.File]::ReadAllText($fp)
Write-Host ("VERIFICA DOPO SCRITTURA: reportsPaginate=" + $v.Contains("reportsPaginate") + " Mostra=" + $v.Contains("Mostra") + " Precedente=" + $v.Contains("Precedente"))